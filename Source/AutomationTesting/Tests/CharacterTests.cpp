...
#include "AutomationTesting/Tests/CharacterTests.h"
...
namespace
{
	const FString CharacterTestBPName =
		"Blueprint'/Game/Tests/BP_Test_ThirdPersonCharacter.BP_Test_ThirdPersonCharacter'";
}
...
bool FHealthMightBeChangedWithDamage::RunTest(const FString& Parameters)
{
	...
	AAutomationTestingCharacter* Character = CreateBlueprintDeferred<AAutomationTestingCharacter>(
		World, CharacterTestBPName, InitialTransform);
...
DEFINE_LATENT_AUTOMATION_COMMAND_TWO_PARAMETER(FCharacterDestroyedLatentCommand, AAutomationTestingCharacter*, Character, float, LifeSpan);
...
bool FCharacterCanBeKilled::RunTest(const FString& Parameters)
{
    ...
    const FTransform InitialTransform{FVector{0.0f, -240.0f, 110.0f}};
    AAutomationTestingCharacter* Character = CreateBlueprintDeferred<AAutomationTestingCharacter>(World, CharacterTestBPName, InitialTransform);
    if (!TestNotNull("Character exists", Character)) return false;
...
DEFINE_LATENT_AUTOMATION_COMMAND_TWO_PARAMETER(FAutoHealCheckLatentCommand, AAutomationTestingCharacter*, Character, float, HealingDuration);
